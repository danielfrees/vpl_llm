...
def subsequent_mask(size):
    ...
    s_mask = torch.triu(torch.ones(attn_shape), diagonal=1).type(
        torch.uint8
    )
    return s_mask == 0
...
def attention(query, key, value, mask=None, dropout=None):
    ...
    d_k = query.size(-1)    # d_k = D/h
    # query, key, value: (B x h x N x D/h)
    # mask: (B x 1 x N)
    scores = torch.matmul(query, key.transpose(-2, -1)) / math.sqrt(d_k)    # (B x h x N x N)
    if mask is not None:
